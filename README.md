## TUNDRA FORK
Добавлена переменная виртуального окружения __SUPPORT_URL__, в которую прописывается адрес ресурса с поддержкой пользователей

Добавлена переменная виртуального окружения __TUNDRA_INFO_BLOCK_ENABLED__ (логическая), для включения информативного блока, что это форк тундры

(Файлы в которых прописан блок: navigation-bar.directive.coffee, navigation-bar.jade, navigation-bar.scss)
